...
const AllOrder = () => {
    ...
    return (
      <div className='h-[calc(100vh-175px)] overflow-y-scroll'>
        {
          !data[0] && (
            <p className='flex justify-center mt-6 font-bold'>No Order Available</p>
          )
        }
        <div className='p-4'>
          {
              ...
              return(
                <div key={item.userId+index} className=''>
                          ...
                          <div key={product.productId+index} className='flex gap-3 bg-slate-100'>
                            ...
                            <div>
                              ...
                              <div className='flex items-center gap-5 mt-1'>
                                <div className='text-lg text-red-500'>{displayLKRCurrency(product.price*300)}</div>
                    ...
                    item.shipping_options.map((shipping,index)=>{
                      return(
                        <div key={shipping.shipping_rate} className=' ml-1'>
                          Shipping Amount : {displayLKRCurrency(shipping.shipping_amount*300)}
                        </div>
                      )
                    })
                  }
                  ...
                  <div className='font-semibold ml-auto w-fit lg:text-lg '>
                    Total Amount : {displayLKRCurrency(item.totalAmount*300)}
                  </div>
                  </div>
                </div>
              )
